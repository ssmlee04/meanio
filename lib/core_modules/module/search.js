...
function readModulePackageJSONFileDone (_modules, fileDefer, file, source, fileErr, data) {
  if (data) {
    try {
      var json = JSON.parse(data.toString());
      if (json.mean) {
        fs.readFile(path.join(process.cwd(), source, file, 'pack.json'), readModuleMeanJSONFileDone.bind(null, _modules, fileDefer, file, source));
        return;
      }
    } catch (err) {
...
function fileForEachProcess (_modules, source, promises, file) {
  ...
  fs.readFile(path.join(process.cwd(), source, file, 'pack.json'), readModulePackageJSONFileDone.bind(null, _modules, fileDefer, file, source));
